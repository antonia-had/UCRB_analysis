...
import math
import sys
# Only uncomment the first time to generate the samples
#sys.path.append('../')
...
MonthlyQ_all = readFiles('../Statemod_files/cm2015x.xbm', 16, 208)
...
# Only uncomment the first time to generate the samples
#param_bounds=np.loadtxt('uncertain_params_original.txt', usecols=(1,2))
#params_no = len(param_bounds)
#param_names=[x.split(' ')[0] for x in open('uncertain_params_original.txt').readlines()]
#problem = {
#    'num_vars': params_no,
#    'names': param_names,
...
#np.savetxt('Sobol_sample.txt', param_values)

samples = np.loadtxt('Sobol_sample.txt',usecols=[0,7,8,9,10,11,12,13])


# Prepend row to samples with base case
base_case = np.ones([1,8])
base_case[0,-3::] = 0
samples = np.concatenate((base_case,samples),0)
...
count = int(math.floor(np.shape(samples)[0]/nprocs))
remainder = np.shape(samples)[0] % nprocs
...
for i in range(start, stop):
    for realization in range(10):
       # create matrices to store nYears of synthetic (_s), real-space (AnnualQ_s), 
        ...
        writeNewFiles('cm2015x.xbm', 16, i, realization+1, MonthlyQ_s, '', design)
        
        # write new irrigation demands to file for LHsample i
        writeNewFiles('cm2015B.iwr', 463, i, realization+1, MonthlyIWR_s, 'a', design) # a for all uncertainties

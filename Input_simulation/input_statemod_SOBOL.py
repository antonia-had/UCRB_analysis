...
param_bounds=np.loadtxt('../Qgen/uncertain_params_original.txt', usecols=(1,2))
params_no = len(param_bounds)
param_names=[x.split(' ')[0] for x in open('../Qgen/uncertain_params_original.txt').readlines()]
problem = {
    'num_vars': params_no,
    'names': param_names,
...
def writenewDDM(structures, firstLine, k, l):    
    ...
    for i in range(len(all_split_data_DDM)-firstLine):
        ...
        if row_data[1]==shoshone[0]: #If the structure is the Shoshone
            ...
            # apply multiplier to 1st month
            row_data[2] = str(int(max_values.loc[row_data[1]][0]*param_values[k,2]))
            # apply multipliers to rest of the columns
            for j in range(1,13):
                row_data.append(str(int(max_values.loc[row_data[1]][j]*param_values[k,2]))) 
        elif row_data[1] in structures[2]: #If the structure is mun_ind 
            # apply multiplier to 1st month
            row_data[2] = str(int(float(row_data[2])*param_values[k,3]))
            # apply multipliers to rest of the columns
            for j in range(len(all_split_data_DDM[i+firstLine])-2):
                row_data.append(str(int(float(all_split_data_DDM[i+firstLine][j+1])*param_values[k,3])))  
...
def writenewRES(lines, k):
    ...
    for j in range(len(lines)):
        ...
        split_line[1] = ' ' + str(int(float(split_line[1])*param_values[k,1]))
...
def writenewEVA(k):
    ...
    for i in range(len(all_split_data_EVA)-35):
        ...
        for j in range(len(all_split_data_EVA[i+35])-1):
            row_data.append(float(all_split_data_EVA[i+35][j+1])+param_values[k,6])            
        # append row of adjusted data
        new_data.append(row_data)            
    # write new data to file

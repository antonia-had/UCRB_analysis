...
def factor_mapping(ID):
    ...
    if ID in TBDs:
        ...
        for j in range(len(fail_duration)):
            for h in range(len(fail_shortage)):
                ...
                for k in range(len(success)):
                    ...
                    if len(shortage_duration(ratio, fail_shortage[j]))>0:
                        if max(shortage_duration(ratio, fail_shortage[j]))>fail_duration[h]:
                            success[k]=0
                dta['Success']=success
        ...
        if ID in irrigation_structures:
            ...
            fail_duration = [50, 20, 10, 1]
            fail_shortage = [5, 10, 20, 50]
        for j in range(len(fail_duration)):
            for h in range(len(fail_shortage)):
                pseudo_r_scores = np.zeros(params_no)
                # Logistic regression analysis
